...
class YoutubeDL(object):
    ...
    def __init__(self, params=None, auto_init=True):
        ...
        def check_deprecated(param, option, suggestion):
            if self.params.get(param) is not None:
                self.report_warning('%s is deprecated. Use %s instead.' %
                                    (option, suggestion))
                return True
            return False

        if check_deprecated('cn_verification_proxy', '--cn-verification-proxy',
        ...
        check_deprecated(
            'autonumber_size', '--autonumber-size',
        ...
        if params.get('bidi_workaround', False):
            try:
                ...
                sp_kwargs = dict(stdin=subprocess.PIPE,
                                 stdout=slave,
    ...
    def trouble(self, message=None, tb=None):
        ...
        if self.params.get('verbose'):
            if tb is None:
                if sys.exc_info(
                )[0]:  # if .trouble has been called from an except block
                    tb = ''
    ...
    def report_warning(self, message):
        ...
        if self.params.get('logger') is not None:
            ...
            if self.params.get('no_warnings'):
                return
            if not self.params.get('no_color') and self._err_file.isatty(
        ...
        try:
            ...
            if template_dict.get('resolution') is None:
                if template_dict.get('width') and template_dict.get('height'):
                    ...
                    template_dict[
                        'resolution'] = '%sp' % template_dict['height']
                elif template_dict.get('width'):
                    template_dict[
                        'resolution'] = '%dx?' % template_dict['width']
            ...
            template_dict = dict(
                (k,
                 v if isinstance(v, compat_numeric_types) else sanitize(k, v))
            ...
            for numeric_field in self._NUMERIC_FIELDS:
                if numeric_field not in template_dict:
                    ...
                    '''
                    outtmpl = re.sub(FORMAT_RE.format(numeric_field),
            ...
            outtmpl = outtmpl.replace('%%', '%{0}%'.format(sep)).replace(
                '$$', '${0}$'.format(sep))
    ...
    def _match_entry(self, info_dict, incomplete):
        ...
        if date is not None:
            ...
            if date not in dateRange:
                return '%s upload date is not in range %s' % (
                    date_from_str(date).isoformat(), dateRange)
        ...
        if age_restricted(info_dict.get('age_limit'),
                          self.params.get('age_limit')):
    ...
    def process_ie_result(self, ie_result, download=True, extra_info={}):
        ...
        if result_type in ('url', 'url_transparent'):
            ...
            if ((extract_flat == 'in_playlist' and 'playlist' in extra_info)
                ...
                self.__forced_printings(ie_result,
                                        self.prepare_filename(ie_result),
                                        incomplete=True)
                return ie_result
    ...
    def build_format_selector(self, format_spec):
        ...
        def _parse_format_selection(tokens,
                                    inside_merge=False,
                                    inside_choice=False,
                                    inside_group=False):
            selectors = []
            ...
            for type, string, start, _, _ in tokens:
                ...
                if type == getattr(tokenize, 'ENCODING', None):
                    ...
                    if string == ')':
                        ...
                        group = _parse_format_selection(tokens,
                                                        inside_group=True)
                        ...
                        if not video_selector or not audio_selector:
                            raise syntax_error(
                                '"+" must be between two format selectors',
                                start)
                        current_selector = FormatSelector(
                            MERGE, (video_selector, audio_selector), [])
                    else:
                        raise syntax_error(
                            'Operator not recognized: "{0}"'.format(string),
                            start)
        ...
        def _build_selector_function(selector):
            if isinstance(selector, list):
                ...
                def _merge(formats_info):
                    ...
                    if formats_info[0].get(
                            'acodec') == 'none' and formats_info[1].get(
                                'acodec') == 'none':
                        self.report_error(
                    ...
                    output_ext = (formats_info[0]['ext']
                                  if self.params.get('merge_output_format') is
                                  None else self.params['merge_output_format'])
                    return {
                        'requested_formats':
                        formats_info,
                        'format':
                        '%s+%s' % (formats_info[0].get('format'),
                                   formats_info[1].get('format')),
                        'format_id':
                        '%s+%s' % (formats_info[0].get('format_id'),
                                   formats_info[1].get('format_id')),
                        'width':
                        formats_info[0].get('width'),
                        'height':
                        formats_info[0].get('height'),
                        'resolution':
                        formats_info[0].get('resolution'),
                        'fps':
                        formats_info[0].get('fps'),
                        'vcodec':
                        formats_info[0].get('vcodec'),
                        'vbr':
                        formats_info[0].get('vbr'),
                        'stretched_ratio':
                        formats_info[0].get('stretched_ratio'),
                        'acodec':
                        formats_info[1].get('acodec'),
                        'abr':
                        formats_info[1].get('abr'),
                        'ext':
                        output_ext,
                    }
                ...
                def selector_function(ctx):
                    for pair in itertools.product(
                            video_selector(copy.deepcopy(ctx)),
                            audio_selector(copy.deepcopy(ctx))):
                        yield _merge(pair)
        ...
        try:
            tokens = list(
                _remove_unused_ops(compat_tokenize_tokenize(stream.readline)))
    ...
    def process_video_result(self, info_dict, download=True):
        ...
        if info_dict.get('upload_date') is None and info_dict.get(
        ...
        for field in ('chapter', 'season', 'episode'):
            if info_dict.get('%s_number' %
                             field) is not None and not info_dict.get(field):
                info_dict[field] = '%s %d' % (field.capitalize(),
        ...
        if self.params.get('listsubtitles', False):
            if 'automatic_captions' in info_dict:
                self.list_subtitles(info_dict['id'], automatic_captions,
                                    'automatic captions')
        ...
        for i, format in enumerate(formats):
            if format.get('format') is None:
                format['format'] = '{id} - {res}{note}'.format(
                    ...
                    note=' ({0})'.format(format['format_note'])
                    if format.get('format_note') is not None else '',
                )
    ...
    def process_subtitles(self, video_id, normal_subtitles,
                          automatic_captions):
        ...
        if (not self.params.get('writesubtitles')
                and not self.params.get('writeautomaticsub')
                or not available_subs):
            return None
        ...
        for lang in requested_langs:
            ...
            if formats is None:
                self.report_warning('%s subtitles not available for %s' %
                                    (lang, video_id))
                continue
    ...
    def __forced_printings(self, info_dict, filename, incomplete):
        ...
        if self.params.get('forceduration',
                           False) and info_dict.get('duration') is not None:
    ...
    def process_info(self, info_dict):
        ...
        def ensure_dir_exists(path):
            try:
                ...
                self.report_error('unable to create directory ' +
        ...
        if self.params.get('writedescription', False):
            descfn = replace_extension(filename, 'description',
                                       info_dict.get('ext'))
            if self.params.get('nooverwrites', False) and os.path.exists(
                ...
                try:
                    self.to_screen('[info] Writing video description to: ' +
                                   descfn)
                    with io.open(encodeFilename(descfn), 'w',
                                 encoding='utf-8') as descfile:
                        descfile.write(info_dict['description'])
                except (OSError, IOError):
                    self.report_error('Cannot write description file ' +
                                      descfn)
                    return
        ...
        if self.params.get('writeannotations', False):
            ...
            if self.params.get('nooverwrites', False) and os.path.exists(
                ...
                try:
                    self.to_screen('[info] Writing video annotations to: ' +
                                   annofn)
                    with io.open(encodeFilename(annofn), 'w',
                                 encoding='utf-8') as annofile:
                        annofile.write(info_dict['annotations'])
                except (KeyError, TypeError):
                    self.report_warning('There are no annotations to write.')
                except (OSError, IOError):
                    self.report_error('Cannot write annotations file: ' +
                                      annofn)
                    return
        ...
        if subtitles_are_requested and info_dict.get('requested_subtitles'):
            ...
            for sub_lang, sub_info in subtitles.items():
                ...
                if self.params.get('nooverwrites', False) and os.path.exists(
                    ...
                    self.to_screen('[info] Writing video subtitles to: ' +
                                   sub_filename)
                    if sub_info.get('data') is not None:
                        try:
                            # Use newline='' to prevent conversion of newline characters
                            # See https://github.com/ytdl-org/youtube-dl/issues/10268
                            with io.open(encodeFilename(sub_filename),
                                         'w',
                                         encoding='utf-8',
                                         newline='') as subfile:
                        ...
                        try:
                            sub_data = ie._request_webpage(sub_info['url'],
                                                           info_dict['id'],
                                                           note=False).read()
                            with io.open(encodeFilename(sub_filename),
                                         'wb') as subfile:
                                subfile.write(sub_data)
                        except (ExtractorError, IOError, OSError,
                                ValueError) as err:
                            self.report_warning(
                                'Unable to download subtitle for "%s": %s' %
                                (sub_lang, error_to_compat_str(err)))
                            continue

        if self.params.get('writeinfojson', False):
            infofn = replace_extension(filename, 'info.json',
                                       info_dict.get('ext'))
            if self.params.get('nooverwrites', False) and os.path.exists(
                ...
                self.to_screen(
                    ...
                    infofn)
                try:
                    write_json_file(self.filter_requested_info(info_dict),
                                    infofn)
                except (OSError, IOError):
                    self.report_error('Cannot write metadata to JSON file ' +
                                      infofn)
                    return
        ...
        if not self.params.get('skip_download', False):
            try:
                ...
                def dl(name, info):
                    ...
                    if self.params.get('verbose'):
                        self.to_stdout('[debug] Invoking downloader on %r' %
                                       info.get('url'))
                ...
                if info_dict.get('requested_formats') is not None:
                    ...
                    def compatible_formats(formats):
                        video, audio = formats
                        # Check extension
                        video_ext, audio_ext = video.get('ext'), audio.get(
                            'ext')
                        if video_ext and audio_ext:
                            COMPATIBLE_EXTS = (('mp3', 'mp4', 'm4a', 'm4p',
                                                'm4b', 'm4r', 'm4v', 'ismv',
                                                'isma'), ('webm'))
                    ...
                    if self.params.get('merge_output_format'
                                       ) is None and not compatible_formats(
                                           requested_formats):
                        info_dict['ext'] = 'mkv'
                        self.report_warning(
            ...
            if success and filename != '-':
                ...
                if (info_dict.get('protocol') == 'm3u8_native'
                    ...
                    if fixup_policy == 'warn':
                        self.report_warning(
                            '%s: malformed AAC bitstream detected.' %
                            (info_dict['id']))
                    elif fixup_policy == 'detect_or_warn':
    ...
    def download(self, url_list):
        ...
        outtmpl = self.params.get('outtmpl', DEFAULT_OUTTMPL)
        if (len(url_list) > 1 and outtmpl != '-' and '%' not in outtmpl
    ...
    def download_with_info_file(self, info_filename):
        ...
        try:
            ...
            if webpage_url is not None:
                self.report_warning(
                    'The info failed to download, trying with "%s"' %
                    webpage_url)
    ...
    def filter_requested_info(info_dict):
        return dict((k, v) for k, v in info_dict.items()
    ...
    def list_thumbnails(self, info_dict):
        ...
        self.to_screen(
            render_table(['ID', 'width', 'height', 'URL'], [[
                t['id'],
                t.get('width', 'unknown'),
                t.get('height', 'unknown'), t['url']
            ] for t in thumbnails]))
    ...
    def print_debug_header(self):
        ...
        stdout_encoding = getattr(sys.stdout, 'encoding',
                                  'missing (%s)' % type(sys.stdout).__name__)
        encoding_str = (
            '[debug] Encodings: locale %s, fs %s, out %s, pref %s\n' %
            (locale.getpreferredencoding(), sys.getfilesystemencoding(),
             stdout_encoding, self.get_encoding()))
        ...
        if _LAZY_LOADER:
            self._write_string('[debug] Lazy loading extractors enabled' +
                               '\n')
        try:
            sp = subprocess.Popen(['git', 'rev-parse', '--short', 'HEAD'],
        ...
        def python_implementation():
            ...
            if impl_name == 'PyPy' and hasattr(sys, 'pypy_version_info'):
                return impl_name + ' version %d.%d.%d' % sys.pypy_version_info[:
                                                                               3]
            return impl_name
        ...
        exe_str = ', '.join('%s %s' % (exe, v)
                            for exe, v in sorted(exe_versions.items()) if v)
        if not exe_str:
            exe_str = 'none'
    ...
    def _setup_opener(self):
        ...
        params['proxy'] = None
        https_handler_download = make_HTTPS_handler(params,
                                                    debuglevel=debuglevel)
        ydlh = YoutubeDLHandler(self.params, debuglevel=debuglevel)
        ydlh_download = YoutubeDLHandler(params, debuglevel=debuglevel)
        ...
        opener = compat_urllib_request.build_opener(proxy_handler,
                                                    https_handler,
                                                    cookie_processor, ydlh,
                                                    redirect_handler,
                                                    data_handler, file_handler)
        opener_download = compat_urllib_request.build_opener(
            # proxy_handler,
            # https_handler,
            PerRequestProxyHandler({}),
            https_handler_download,
            cookie_processor,
            ydlh_download,
            redirect_handler,
            data_handler,
        ...
        opener.addheaders = []
        opener_download.addheaders = []
        self._opener = opener
        self._opener_download = opener_download
            ...
            if self.params.get('nooverwrites', False) and os.path.exists(
                ...
                try:
                    ...
                    self.to_screen('[%s] %s: Writing thumbnail %sto: %s' %
                                   (info_dict['extractor'], info_dict['id'],
                                    thumb_display_id, thumb_filename))
                except (compat_urllib_error.URLError,
                        compat_http_client.HTTPException, socket.error) as err:
                    self.report_warning(
